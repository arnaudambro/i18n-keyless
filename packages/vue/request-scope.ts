...
// a pure Set.add with NO store write / setState, so it can't trigger Vue's
...
//     `getRequestScope`/`recordUsedKey` (Vue render) read copy B's `als`: `undefined` :
...
//     and the Vue render in two different V8 realms that SHARE the same `globalThis` object
  ...
  if (!registry[ALS_INIT_KEY]) {
    registry[ALS_INIT_KEY] = (async () => {
      try {
        ...
        // (guarded by the `typeof window` check above) and is wrapped in try/catch, so the browser and edge
        // runtimes without it degrade to a no-op exactly as before.
