...
export interface I18nConfig {
   ...
   *
   * you can use window.localStorage, idb-keyval for IndexedDB, Capacitor Preferences, or any storage that has a getItem, setItem, removeItem, or get, set, and remove method
   */
  storage?: {
...
export interface TranslationStoreState {
  ...
  /**
   * Bumped whenever core's pending-translation set changes (a miss queued, or settled by
   * the bulk fetch that follows it). Pure bookkeeping: nothing else in the store reads its
   * value, but `useTranslationStatus()` / `tStatus()` read it inside their `computed` so
   * Vue re-evaluates status when a key's pending flag flips.
   */
